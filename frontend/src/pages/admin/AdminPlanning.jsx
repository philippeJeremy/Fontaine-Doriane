...
const HOUR_PX    = 64;  // hauteur d'une heure en pixels
const START_HOUR = 7;
const END_HOUR   = 22;
...
// ── Vue journée ───────────────────────────────────────────────────────────────
function DayView({ dateStr, appointments, onOpenDetail, onOpenCreate }) {
  ...
  function topPx(timeStr) {
    return ((timeToMin(timeStr) - START_HOUR * 60) / 60) * HOUR_PX;
  }

  function heightPx(startStr, endStr) {
    return Math.max(((timeToMin(endStr) - timeToMin(startStr)) / 60) * HOUR_PX, 28);
  }

  function handleGridClick(e) {
    const rect = e.currentTarget.getBoundingClientRect();
    const relY  = e.clientY - rect.top;
    const totalMin = (relY / HOUR_PX) * 60 + START_HOUR * 60;
    const h = Math.floor(totalMin / 60);
    const m = Math.round((totalMin % 60) / 15) * 15;
    const snapped = `${String(Math.min(h, END_HOUR - 1)).padStart(2, "0")}:${String(m % 60).padStart(2, "0")}`;
    onOpenCreate(dateStr, snapped);
  }

  return (
    <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden select-none">
      <div className="flex" style={{ height: `${totalHeight}px` }}>

        {/* Colonne heures */}
        <div className="w-14 shrink-0 border-r border-stone-100 relative">
          {Array.from({ length: END_HOUR - START_HOUR }, (_, i) => (
            <div key={i} className="absolute left-0 right-0 flex items-start px-2 pt-1"
              style={{ top: `${i * HOUR_PX}px`, height: `${HOUR_PX}px` }}>
              <span className="text-xs text-stone-300 font-medium leading-none">
        ...
        {/* Zone rendez-vous */}
        <div className="flex-1 relative cursor-pointer" onClick={handleGridClick}>
          {/* Lignes heures */}
          {Array.from({ length: END_HOUR - START_HOUR }, (_, i) => (
          ...
          {/* Lignes demi-heures */}
          {Array.from({ length: END_HOUR - START_HOUR }, (_, i) => (
            <div key={`h${i}`} className="absolute left-0 right-0 border-t border-stone-50"
              style={{ top: `${i * HOUR_PX + HOUR_PX / 2}px` }} />
          ))}

          {/* Blocs RDV */}
          {dayAppts.map(a => {
            const s = STATUS[a.status] ?? { label: a.status, cls: "bg-stone-100 text-stone-500" };
            const top = topPx(a.start_time);
            const h   = heightPx(a.start_time, a.end_time);
            return (
              <div
                key={a.id}
                className={`absolute left-2 right-2 rounded-lg px-2.5 py-1.5 cursor-pointer hover:opacity-90 transition overflow-hidden shadow-sm border border-white/60 ${s.cls}`}
                style={{ top: `${top}px`, height: `${h}px` }}
                onClick={e => { e.stopPropagation(); onOpenDetail(a); }}
              >
                <p className="text-xs font-bold leading-tight">
                  {a.start_time} – {a.end_time}
                  {a.is_home_service && <span className="ml-1">🏠</span>}
                </p>
                <p className="text-xs font-semibold truncate leading-tight mt-0.5">{clientName(a)}</p>
                {h > 50 && (
                  <p className="text-xs truncate leading-tight opacity-70 mt-0.5">
                    {a.services[0]?.name}{a.services.length > 1 ? ` +${a.services.length - 1}` : ""}
                  </p>
                )}
                {h > 70 && (
                  <p className="text-xs leading-tight opacity-60 mt-0.5 font-medium">
          ...
          {dayAppts.length === 0 && (
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <p className="text-stone-200 text-sm">Aucun rendez-vous</p>
              <p className="text-stone-200 text-xs mt-1">Cliquer pour ajouter</p>
            </div>
          )}
        </div>
      </div>
...
function CreatePanel({ initialDate, initialTime, allServices, onClose, onCreated }) {
...
export default function AdminPlanning() {
  ...
  // Sync year/month lorsqu'on navigue dans la vue jour vers un autre mois
  useEffect(() => {
  ...
  function prevDay() {
    ...
    setDayStr(d.toISOString().slice(0, 10));
  }
  function nextDay() {
    const d = new Date(dayStr + "T00:00:00");
    d.setDate(d.getDate() + 1);
  ...
  const dayLabel = new Date(dayStr + "T00:00:00").toLocaleDateString("fr-FR", {
    weekday: "long", day: "numeric", month: "long", year: "numeric",
  });
  ...
  function handleCreated(newAppt) {
    const apptDate  = new Date(newAppt.date + "T00:00:00");
  ...
  function openCreate(date, time = null) {
    setCreateDate(date);
    setCreateTime(time);
    setShowCreate(true);
  }
    ...
    <div>
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between gap-3 mb-3">
          ...
          <button
            onClick={() => openCreate(view === "day" ? dayStr : todayStr)}
            className="bg-sauge-500 hover:bg-sauge-600 text-white text-sm font-medium px-5 py-2 rounded-full transition shrink-0"
          >
            + Nouveau RDV
          </button>
        </div>
        {/* Toggle vue — ligne séparée pour mobile */}
        <div className="flex gap-1 bg-stone-100 p-1 rounded-full w-fit">
          <button
            onClick={() => setView("month")}
            className={`text-sm px-4 py-1.5 rounded-full font-medium transition ${view === "month" ? "bg-white shadow text-sauge-600" : "text-stone-500 hover:text-stone-700"}`}
          >
            Mois
          </button>
          <button
            onClick={() => setView("day")}
            className={`text-sm px-4 py-1.5 rounded-full font-medium transition ${view === "day" ? "bg-white shadow text-sauge-600" : "text-stone-500 hover:text-stone-700"}`}
          >
            Jour
          </button>
        </div>
      ...
      {view === "month" ? (
        ...
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          <button onClick={prevDay} className="w-8 h-8 rounded-full hover:bg-sauge-50 text-stone-500 text-lg transition flex items-center justify-center">‹</button>
          <input
            type="date"
            value={dayStr}
            onChange={e => e.target.value && setDayStr(e.target.value)}
            className="border border-stone-200 rounded-xl px-3 py-1.5 text-sm text-stone-700 font-medium focus:outline-none focus:ring-2 focus:ring-sauge-200 capitalize"
          />
          <button onClick={nextDay} className="w-8 h-8 rounded-full hover:bg-sauge-50 text-stone-500 text-lg transition flex items-center justify-center">›</button>
          <span className="text-stone-500 text-sm font-medium capitalize hidden sm:inline">{dayLabel}</span>
          <button
            onClick={() => setDayStr(todayStr)}
            className="ml-1 text-xs border border-stone-200 text-stone-500 px-3 py-1 rounded-full hover:bg-stone-50 transition"
          >
            Aujourd'hui
          </button>
          {loading && <div className="w-4 h-4 border-2 border-sauge-200 border-t-sauge-500 rounded-full animate-spin ml-1" />}
      ...
      {view === "month" && (
        <>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`text-xs font-semibold w-5 h-5 flex items-center justify-center rounded-full cursor-pointer hover:ring-2 hover:ring-sauge-300 transition ${
                          isToday ? "bg-sauge-500 text-white" : isPast ? "text-stone-300" : "text-stone-500"
                        }`}
                        onClick={e => { e.stopPropagation(); switchToDay(dateStr); }}
                    ...
                    <div className="space-y-1">
                      ...
                      {dayAppts.length > 2 && (
                        <p
                          className="text-xs text-stone-400 pl-1 cursor-pointer hover:text-sauge-500 transition"
                          onClick={e => { e.stopPropagation(); switchToDay(dateStr); }}
                        >
                          +{dayAppts.length - 2} autre{dayAppts.length - 2 > 1 ? "s" : ""} — voir jour
                        </p>
                      )}
                    </div>
                  </div>
          ...
          <div className="flex flex-wrap gap-4 mt-3 text-xs text-stone-400">
            ...
            <span className="text-stone-300 hidden sm:inline">— Cliquer sur le numéro d'un jour pour voir la vue journée</span>
          </div>
        </>
      )}
      ...
      {view === "day" && (
        <>
          ...
          <div className="flex flex-wrap gap-4 mt-3 text-xs text-stone-400">
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-amber-100 inline-block border border-amber-200" />En attente</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-green-100 inline-block border border-green-200" />Confirmé</span>
            <span className="text-stone-300 hidden sm:inline">— Cliquer sur la timeline pour créer un RDV à cette heure</span>
          </div>
        </>
      )}
