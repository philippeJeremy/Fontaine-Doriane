...
export default function AdminCalendrier() {
  ...
  const [saveOk, setSaveOk] = useState(null); // day_of_week du dernier save réussi
  ...
  async function updateHours(index, field, value) {
    const updated = hours.map((h, i) => i === index ? { ...h, [field]: value } : h);
    setHours(updated);
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <section className="bg-white rounded-2xl border border-sauge-100 shadow-sm p-6">
        ...
        <div className="space-y-4">
          {hours.map((h, i) => (
            <div key={h.day_of_week} className="flex items-center gap-4 flex-wrap">
              ...
              {/* Toggle ouvert/fermé */}
              <button
                onClick={() => updateHours(i, "is_open", !h.is_open)}
                ...
                <>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={e => updateHours(i, "open_time", e.target.value)}
                      className="border border-stone-200 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-sauge-200"
                    />
                    <span className="text-stone-400 text-sm">→</span>
                    <input
                      type="time"
                      value={h.close_time ?? "19:00"}
                      onChange={e => updateHours(i, "close_time", e.target.value)}
                      className="border border-stone-200 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-sauge-200"
                    />
                  </div>
          ...
          {closedDays.map(d => (
            <li key={d.id} className="flex items-center justify-between bg-sauge-50 rounded-xl px-4 py-3">
              <div>
                ...
                {d.reason && (
                  <span className="text-stone-500 text-xs ml-2">— {d.reason}</span>
                )}
              </div>
              <button
