        ...
        {selectedDate && !loadingSlots && slots.length > 0 && (() => {
          ...
          return (
            <div>
              ...
              {!hasAvailable && (
                <p className="text-red-400 text-sm mb-3">Aucun créneau libre ce jour — essayez une autre date.</p>
              )}
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-1.5">
                {slots.map(slot => {
                  const isSelected = slot.time === selectedSlot;
                  const available  = slot.status === "available";
                  const occupied   = slot.status === "unavailable";
                  return (
                    <button
                      key={slot.time}
                      disabled={!available}
                      onClick={() => available && onSlot(slot.time === selectedSlot ? null : slot.time)}
                      title={occupied ? "Créneau occupé" : slot.status === "no_fit" ? "Fin de journée" : undefined}
                      className={`py-1.5 px-2 rounded-lg text-xs font-medium border transition
                        ${isSelected  ? "bg-sauge-500 text-white border-sauge-500" :
                          available   ? "bg-white text-stone-700 border-stone-200 hover:border-sauge-400 hover:bg-sauge-50 cursor-pointer" :
                          occupied    ? "bg-red-50 text-red-400 border-red-100 cursor-not-allowed" :
              ...
              <div className="flex flex-wrap gap-4 mt-4 text-xs">
                <span className="flex items-center gap-1.5 text-stone-500">
                  <span className="w-3 h-3 rounded border-2 border-stone-300 bg-white inline-block"></span>
                  Disponible
                </span>
                <span className="flex items-center gap-1.5 text-sauge-600">
                  <span className="w-3 h-3 rounded bg-sauge-500 inline-block"></span>
                  Sélectionné
                </span>
                ...
                <span className="flex items-center gap-1.5 text-stone-400">
                  <span className="w-3 h-3 rounded bg-stone-200 border border-stone-300 inline-block"></span>
                  Hors horaires
                </span>
              </div>
            </div>
